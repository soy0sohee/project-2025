...
import com.study.Ex16Security01.serivce.CustomOAuth2userService;
...
public class SecurityConfig {
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {  // 시큐리티가 발생하면 그쪽에서 처리해라
        ...
        http
                // CSRF 보안 설정을 비활성화(개발편의시) + 리액트랑 쓸 때
                ...
                .authorizeHttpRequests((authorization) -> authorization
                        ...
                        .requestMatchers("/joinForm","/joinAction").permitAll()
                ...
                // .formLogin 설정 시 로그인 성공 -> 인증 객체 생성 -> session 저장
                ...
                // SNS 로그인 설정
                // 람다매개변수 타입 : OAuth2LoginConfigurer<HttpSecurity>
    ...
    // 접근제한자 -> 클래스나 함수, 변수 앞에 위치하며, 접근 권한을 제어함
    // public : 다른폴더(패키지) + 상속관계 + 같은폴더(패키지) + 자기클래스
    // protected : 상속관계 + 같은폴더(패키지) + 자기클래스
    // default(생략가능) : 같은폴더(패키지) + 자기클래스
    // private : 자기클래스

    @Bean  // GET 방식
    SimpleUrlAuthenticationSuccessHandler successHandler() {
        // URL은 SnsController가 처리
