...
export function Counter() {
  ...
  // let 대신 const 사용하는 이유
  // : 상태 변수를 임의로 변경 불가, 리액트가 직접 관리하고 있기 때문에
...
// 입력된 텍스트 화면에 바로 보여주기
// : 입력 필드에 사용자가 입력한 텍스트를 따로 화면에 보여줌
// : onChange이벤트 사용
export function TextMirror() {
  const [text, setText] = useState(""); // 빈문자열 초기화
  ...
  return (
    <>
      <input
        type="text"
        value={text}
        placehorder="텍스트를 입력하세요"
        // 이벤트 함수는 매개변수로 event객체를 받을 수 있음
        // e : event객체 -> 이벤트 정보를 담음
        // e.target : input객체 -> 이벤트가 발생하는 객체
        onChange={(e) => {
